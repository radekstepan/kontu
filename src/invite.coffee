    ...
    post: (req, res) =>
        ...
        # Check the format of req and if the other user exists.
            ...
            collections.users.findOne { 'id': user_id }, (err, doc) ->
                ...
                if !doc then return def.reject { 'code': 403, 'message': "User `#{user_id}` not found" }
                def.resolve [ user1, doc, collections ]
            def.promise

        # Save the debtor, creditor accounts.
        ).then( ([ user1, user2, collections ]) ->
            # Just make sure we have the default currency set.
            unless user1.currency
                throw "User `#{user1.id}` does not have a default currency set"
            ...
            # Just make sure they have the default currency set.
            unless user2.currency
                throw "User `#{user2.id}` does not have a default currency set"
